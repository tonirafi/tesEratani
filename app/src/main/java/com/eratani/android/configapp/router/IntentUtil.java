...
//            ((PermissionActivity) context).requestPermissions(RS_CODE, Permission.CAMERA, Permission.STORAGE);
    ...
    public static void intentToJualProduk(Context context, @NonNull Object produk) {

        Uri.Builder builder = RouterConstants.URI.INSTANCE.getJUAL_PRODUK().buildUpon();
        if (produk instanceof ProdukModel) {
    ...
    public static void intentToUpdateProduk(Context context, @NonNull Object produk) {

        Uri.Builder builder = RouterConstants.URI.INSTANCE.getUPDATE_PRODUK().buildUpon();
        if (produk instanceof ProdukModel) {
            builder.appendQueryParameter(RouterConstants.Params.PRE_LOAD, AppUtil.getGsonInstance().toJson(produk));
        }
